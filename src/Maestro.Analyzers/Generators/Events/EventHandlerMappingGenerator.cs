	...
	public void Initialize(IncrementalGeneratorInitializationContext context)
	{
        ...
        context.RegisterSourceOutput(records, (context, input) =>
		{
			...
			var matches = items.Where(x => x?.SyntaxTree is not null).Select(x => compilation.GetSemanticModel(x!.SyntaxTree)?.GetDeclaredSymbol(x))
				...
				.Select(x => new EventHandlerMapping(x, x.AllInterfaces.FirstOrDefault(x => x.OriginalDefinition?.ToDisplayString() == eventHandlerInterface)?.TypeArguments.FirstOrDefault()))
				.Where(x => x.TypeArgument is not null)
				.ToImmutableArray();
			if (matches.Any())
			{
                var body = string.Join("\n        ", matches.Select(x => @$"services.AddSingleton<IEventHandler<global::{x.TypeArgument!.ToDisplayString()}>, global::{x!.EventHandlerSymbol.ToDisplayString()}>();"));
