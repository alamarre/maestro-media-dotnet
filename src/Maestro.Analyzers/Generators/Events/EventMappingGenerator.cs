    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(records, (context, input) =>
        {
            ...
            var matches = items.Where(x => x?.SyntaxTree is not null)
                ...
                .Cast<ITypeSymbol>()
                .Select(x => new EventMapping(x,
                    x.GetAttributes().FirstOrDefault(x => x.AttributeClass?.ToDisplayString() == eventAttribute)
            ...
            {
                if (Guid.TryParse(match.EventId, out _))
                {
                    continue;
                }
                ...
                context.ReportDiagnostic(Diagnostic.Create(
                    new DiagnosticDescriptor(
                        "MAE001",
                        "Event Ids must be a GUID",
                        "Event Ids must be a GUID",
                        "Maestro",
                        DiagnosticSeverity.Error,
                        true
                    ),
                    match.EventSymbol.Locations.First()
                ));
            }
            ...
            if (matches.Any())
            {
                var body = string.Join(",\n            ",
                    matches.Select(x =>
                        @$"{{ Guid.Parse(""{x.EventId}""), typeof(global::{x.EventSymbol.ToDisplayString()}) }}"));
...
public static partial class AutoEventMapping {{
    static partial void Initialize() {{
        IdsToEvents = new Dictionary<Guid, global::System.Type> {{
            {body}
