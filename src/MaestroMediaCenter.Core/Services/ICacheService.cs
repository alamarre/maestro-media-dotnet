...
public class DbCacheSerice(IDbContextFactory<MediaDbContext> dbContextFactory) : ICacheService
{
    public async Task<VideoCache> GetCacheAsync(CancellationToken cancellationToken = default)
    {
        ...
        var moviesMap = videos.Where(v => v.VideoType == VideoType.Movie)
            .ToDictionary(v => v.VideoName, _ => string.Empty);
        ...
        foreach (var episode in tvShows)
        {
            ...
            {
                currentShow.Folders.Add(season,
                    new(new Dictionary<string, string>(), new Dictionary<string, VideoCache>()));
            }
        ...
        VideoCache root = new(new Dictionary<string, string>(),
            new Dictionary<string, VideoCache>
            {
