    ...
    IDbContextFactory<MediaDbContext> dbContextFactory) : IMetadataService
{
    ...
    async Task IMetadataService.FetchMetadata(Guid videoId, CancellationToken cancellationToken)
    {
        ...
        var existingMetadata =
            await db.VideoTmdbMetadata.FirstOrDefaultAsync(
                x => x.VideoType == video.VideoType && x.VideoName == video.VideoName, cancellationToken);
        ...
        {
            var searchResult =
                await client.SearchTvShowAsync(name, firstAirDateYear: year, cancellationToken: cancellationToken);
            if (searchResult.Results.Count == 0)
            {
                await db.VideoTmdbMetadata.AddAsync(
                    new VideoTmdbMetadata
                    {
