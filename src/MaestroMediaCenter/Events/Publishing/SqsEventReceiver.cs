    ...
    )  : IEventReceiver
{
    ...
    {
        await sqsClient.DeleteMessageAsync(new Amazon.SQS.Model.DeleteMessageRequest {
    ...
    {
        var result = await sqsClient.ReceiveMessageAsync(new Amazon.SQS.Model.ReceiveMessageRequest {
            QueueUrl = eventOptions.SqsQueueUrl,
